...
Code is from: https://github.com/daniellevy/fast-dro/
"""
...
import numpy as np
import logging
from utils_dro import project_to_cs_ball, project_to_cvar_ball
...
class CustomDistributionSampler(Sampler):
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            # to make absolutely sure that p is in the simplex
...
def chi_square_value(p, v, reg):
    ...
    with torch.no_grad():
        chi2 = (0.5 / m) * reg * (torch.norm(m * p - torch.ones(m, ).to(p.device), p=2) ** 2)
...
class PrimalDualRobustLoss(nn.Module):
    ...
    def forward(self, v, update=True):
        if update and not self.is_erm:
            ...
            coefs = self.step_size / (len(v) * self.sampler.p[self.sampler.inds])
            # pdb.set_trace()
